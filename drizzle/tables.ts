...
export const contact_methods = ["email", "phone", "sms", "whatsapp", "video call"] as const;
...
export const users = sqliteTable("users", {
  ...
  preferred_contact_method: text("preferred_contact_method").$type<typeof contact_methods[number]>(),
...
export const treatment_timelines = ["immediate", "within a month", "within 3 months", "within 6 months", "not sure", "researching"] as const;
...
export const desired_procedures = sqliteTable("desired_procedures", {
  ...
  clerk_id: text("clerk_id").references(() => users.clerk_id).notNull(),
  procedure_id: integer("procedure_id").references(() => procedures.id).notNull(),
  notes: text("notes"),
  createdAt: integer("created_at", { mode: "timestamp" }).notNull().$default(() => new Date()),
  updatedAt: integer("updated_at", { mode: "timestamp" }).notNull().$default(() => new Date()),
